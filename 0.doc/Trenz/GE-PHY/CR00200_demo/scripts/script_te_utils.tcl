  ...
  namespace eval UTILS {
   ...
   # puts "----------Test:$typ | [file exists ${baseFolder}/${osPath}/${shortDir}/${appPath}]|${baseFolder}/${osPath}/${shortDir}/${appPath} "
    ...
    proc vitis_z_bif {{args ""}} {
      ...
      #TOTOD remove  uboot_dtb and bscr --> this will be put to data...(more flexible)
      ...
      for {set option 0} {$option < $args_cnt} {incr option} {
        switch [lindex $args $option] { 
          ...
          "-uboot_dtb_offset"       { incr option; set uboot_dtb_offset [lindex $args $option]}
          ...
          "-bscr_offset"              { incr option; set bscr_offset [lindex $args $option]}
      ...
      if {$linux_bif == true  && $uboot_dtb_use == true } {
         if {[string match "" "$uboot_dtb_offset"] || [string match "NA" "$uboot_dtb_offset"]} {
          puts -nonewline $bif_fp "  \[load = $uboot_dtb_load\]"
        } else {
          if {[string match "" "$uboot_dtb_load"] || [string match "NA" "$uboot_dtb_load"]} {
            puts -nonewline $bif_fp "  \[offset =  $uboot_dtb_offset\]"
          } else {
            puts -nonewline $bif_fp "  \[offset =  $uboot_dtb_offset,load =  $uboot_dtb_load\]"
          }   
        } 
      ...
      if {$linux_bif == true  && $bscr_use == true } {
        if {[string match "" "$bscr_offset"] || [string match "NA" "$bscr_offset"]} {
          puts -nonewline $bif_fp "  \[load = $bscr_load\]"
        } else {
          if {[string match "" "$bscr_load"] || [string match "NA" "$bscr_load"]} {
            puts -nonewline $bif_fp "  \[offset =  $bscr_offset\]"
          } else {
            puts -nonewline $bif_fp "  \[offset =  $bscr_offset,load =  $bscr_load\]"
          }   
        }  
        puts $bif_fp $bscr
      }
    ...
    proc vitis_zmp_bif {{args ""}} {
      ...
      #app extension(uboot device tree)
      if {$linux_bif == true  && $uboot_dtb_use == true } {
         if {[string match "" "$uboot_dtb_offset"] || [string match "NA" "$uboot_dtb_offset"]} {
          puts -nonewline $bif_fp "  \[destination_cpu = $app_cpu, load = $uboot_dtb_load\]"
        } else {
          if {[string match "" "$uboot_dtb_load"] || [string match "NA" "$uboot_dtb_load"]} {
            puts -nonewline $bif_fp "  \[destination_cpu = $app_cpu, offset =  $uboot_dtb_offset\]"
          } else {
            puts -nonewline $bif_fp "  \[destination_cpu = $app_cpu, offset =  $uboot_dtb_offset,load =  $uboot_dtb_load\]"
          }   
        }   
        puts $bif_fp $uboot_dtb
      }
      ...
      if {$linux_bif == true  && $bscr_use == true } {   
        if {[string match "" "$bscr_offset"] || [string match "NA" "$bscr_offset"]} {
          puts -nonewline $bif_fp "  \[destination_cpu = $app_cpu,load = $bscr_load\]"
        } else {
          if {[string match "" "$bscr_load"] || [string match "NA" "$bscr_load"]} {
            puts -nonewline $bif_fp "  \[destination_cpu = $app_cpu,offset =  $bscr_offset\]"
          } else {
            puts -nonewline $bif_fp "  \[destination_cpu = $app_cpu,offset =  $bscr_offset,load =  $bscr_load\]"
          }   
        }    
        puts $bif_fp $bscr
      }
